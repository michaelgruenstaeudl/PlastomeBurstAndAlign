...
subprocess.run(["python3", "-m", "plastburstalign",
                "-i", main_folder, "-o", folder_CDS, "-s", "cds", "-t", "5", "-l", "6", "-n", nCPUs])
run_end = time.time()

# Step 5: Organize the output files
subprocess.run(["mv"] + glob.glob(f"{folder_CDS}/*.unalign.fasta") + [f"{folder_CDS}/01_unalign/"])
subprocess.run(["mv"] + glob.glob(f"{folder_CDS}/*.aligned.fasta") + [f"{folder_CDS}/02_aligned/fasta/"])
subprocess.run(["mv"] + glob.glob(f"{folder_CDS}/*.aligned.nexus") + [f"{folder_CDS}/02_aligned/nexus/"])
